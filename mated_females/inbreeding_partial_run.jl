...
function fixedmig_TSR_mated(s,hD,hN,σ,δ,Rm,K,xD,yD,ϵ,ν,μ,β,ξ,width::Int,height::Int,pmig,pmat,preexisting,T,test_threshold,initial_run,mated_females,females,males,female_genotype_offspring,male_genotype_offspring)
    ...
    if preexisting == 1 && inital_run == true
    ...
    PP = zeros(size(popsize)[4]);
    for i = 1:size(PP)[1]
        PP[i] = sum(popsize[3,:,:,i]);
    end

    return (PP,mated_females,females,males,female_genotype_offspring,male_genotype_offspring,resistance,τ)

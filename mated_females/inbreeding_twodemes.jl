...
# Add function to implement random walk in one dimension.
function one_dim_migration(v,pmig);
    # v must be vector, not general array
    if length(v) != size(v)[1]
        print("Error - this migation code is for a one dimensional random walk. Input must be vector or nx1 array.")
        return
    end
    len = size(v)[1];

    # Must have integer entries.
    v_int = zeros(Int,len);
    v_int[:] = v[:];
    ...
    for i = 1:len
        if i == 1
            migrants = rand(Multinomial(v_int[i],[1-pmig/2;pmig/2]));
            w[i] += migrants[1];
            w[i+1] += migrants[2];
        elseif i == len
            migrants = rand(Multinomial(v_int[i],[pmig/2;1-pmig/2]));
            w[i-1] += migrants[1];
            w[i] += migrants[2];
        else
            migrants = rand(Multinomial(v_int[i],[pmig/2;1-pmig;pmig/2]));
            w[i-1] += migrants[1];
            w[i] += migrants[2];
            w[i+1] += migrants[3];
        end
...
function fixedmig_TSR_mated_one_dim(s,hD,hN,σ,δ,Rm,K,xD,yD,ϵ,ν,μ,β,ξ,length,pmig,pmat,preexisting,T,test_threshold)
    width = length;
    height = 1;
    ...
    females = zeros(Int,length,n);
    males = zeros(Int,length,n);
    for i = 1:width, j=1:height
        females[i,1] = rand(Poisson(0.5*K_perdeme));
        males[i,1] = rand(Poisson(0.5*K_perdeme));
    end

    mated_females = zeros(length,n^2,2); # Throw away unmated females to save memory
    ...
    female_genotype_offspring = zeros(length,n^2,n);
    male_genotype_offspring = zeros(length,n^2,n);
    ...
    xnum = zeros(length,r,T);
    ynum = zeros(length,r,T);
    x = zeros(length,r,T);
    y = zeros(length,r,T);
    popsize = zeros(3,length,T);
    # Initialise resistance, τ
    resistance = 0;
    τ = NaN;

    for i = 1:width, j = 1:height
        popsize[:,i,1] = [sum(females[i,:]);sum(males[i,:]);sum(females[i,:]+males[i,:])];
    end
    ...
    for i = 1:width, j = 1:height, kk = 1:n
        xnum[i,genotype_list[kk,1],1] += females[i,kk];
        xnum[i,genotype_list[kk,2],1] += females[i,kk];
        ynum[i,genotype_list[kk,1],1] = males[i,kk];
        ynum[i,genotype_list[kk,2],1] = males[i,kk];
    end
    ...
    for i = 1:width, j = 1:height
        ...
        females[i,7] += ceil(Int, 2*popsize[1,i,1]*xD/(1-2*xD));
        males[i,7] += ceil(Int, 2*popsize[2,i,1]*yD/(1-2*yD));
    ...
    for t = 1:T-1
        ...
        if t == 1
            for i=1:width, j=1:height
                mated_females[i,:,1] = mating_non_inbred(females[i,:],males[i,:],pmat)[1:end-1];
            end
        else
            for i=1:width, j=1:height
                mated_females[i,:,:] = mating(females[i,:],males[i,:],female_genotype_offspring[i,:,:],male_genotype_offspring[i,:,:],mated_females[i,:,1]+mated_females[i,:,2],pmat)[1:end-1 , :];
            end
        end
        ### Throw away females which did not find a mate.
        ###mated_females = mated_females[1:end-1];
        ...
        # Mated females migrate
        for ii = 1:n^2, jj = 1:2
            mated_females[:,ii,jj] = one_dim_migration(mated_females[:,ii,jj],pmig);
        end
        # Now for females and males in next generation
        # Reset offspring arrays
        female_genotype_offspring = zeros(width,n^2,n);
        male_genotype_offspring = zeros(width,n^2,n);
        for i = 1:width, j = 1:height
            totfem = sum(females[i,:]);
            females[i,:] = zeros(Int,n);
            males[i,:] = zeros(Int,n);
            for kk = 1:n^2
                (ff,mm) = offspring_production(kk,mated_females[i,kk,1],totfem,Rm,K_perdeme,fitness_vector,0,gamete_array);
                female_genotype_offspring[i,kk,:] += ff;
                male_genotype_offspring[i,kk,:] += mm;
                females[i,:] += ff;
                males[i,:] += mm;
                (ff,mm) = offspring_production(kk,mated_females[i,kk,2],totfem,Rm,K_perdeme,fitness_vector,δ,gamete_array);
                female_genotype_offspring[i,kk,:] += ff;
                male_genotype_offspring[i,kk,:] += mm;
                females[i,:] += ff;
                males[i,:] += mm;
            ...
            popsize[:,i,t+1] = [sum(females[i,:]);sum(males[i,:]);sum(females[i,:]+males[i,:])];
        end

        # Each allele contributes 1 towards xnum. So an RR individual contributes 2 to xnum[2,t].
        # This means that sum(xnum[:,t]) = 2*sum(females). We will halve xnum, ynum at the end to correct for this.
        for i = 1:width, j = 1:height, kk = 1:n
            xnum[i,genotype_list[kk,1],t+1] += females[i,kk];
            xnum[i,genotype_list[kk,2],t+1] += females[i,kk];
            ynum[i,genotype_list[kk,1],t+1] = males[i,kk];
            ynum[i,genotype_list[kk,2],t+1] = males[i,kk];
        end
        ...
        for i = 1:width, j = 1:height
            ...
            if maximum(x[i,:,t+1]) > 0
                x[i,:,t+1] = x[i,:,t+1]/sum(x[i,:,t+1]);
            end
            if maximum(y[i,:,t+1]) > 0
                y[i,:,t+1] = y[i,:,t+1]/sum(y[i,:,t+1]);
            end
        end

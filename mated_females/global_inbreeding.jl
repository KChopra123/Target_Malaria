...
function global_migration(C)
    # Each individual in the array C ends up in one entry of C, randomly uniformly.
    D = zeros(Int,size(C));
    ...
    w = rand(Multinomial(NN,v));
    for i = 1:acrm, j = 1:upn
        D[i,j] = w[i + acrm*(j-1)];
    end
...
function globalmig_TSR_mated(s,hD,hN,σ,δ,Rm,K,xD,yD,ϵ,ν,μ,β,ξ,width::Int,height::Int,pmig,pmat,preexisting,T,test_threshold)
    ...
    if preexisting == 1
        for t = 1:ceil(Int,5/σ)   # Longer burn-in time than initial program as we don't start from an estimate of the standing variation
            ...
            for ii = 1:n^2
                mated_females[:,:,ii] = global_migration(mated_females[:,:,ii]);
            end
    ...
    for t = 1:T-1
        ...
        # Mated females migrate
        for ii = 1:n^2, jj = 1:2
            mated_females[:,:,ii,jj] = global_migration(mated_females[:,:,ii,jj]);
        end
        # Now for females and males in next generation

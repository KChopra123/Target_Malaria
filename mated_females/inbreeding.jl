...
function mating(females,males,pmat,previous_mated_females)
    ...
    prob_vec = zeros(n+1);
    for k = 1:n
        prob_vec[k] = (1 - (1 - pmat)^tot_males) * sum(males[k,:])/tot_males;
    end
    ...
    for i = 1:n
        ...
        mated_females[n^2+1,1] += v[end];
    end

    p_sib = zeros(n^2);
    for i = 1:n, j=1:n, k=1:n^2
        if sum(females[i,:]) > 0 && sum(males[j,:]) > 0
            sf = 0;
            sm = 0;
            for iii = 1:n
                sf += females[genotype_position(i,k,n)];
                sm += males[genotype_position(j,k,n)];
            end
            p_sib[genotype_position(i,j,n)] += (females[genotype_position(i,k,n)]/sum(females[i,:])) * (males[j,k]/sum(males[j,:])) * (1/previous_mated_females[k]);
        end
    end

    for k = 1:n^2
        mated_females[k,:] = rand(Multinomial(mated_females[k,1],[1-p_sib[k],p_sib[k]]));
    end
...
function fixedmig_TSR_mated(s,hD,hN,σ,Rm,K,xD,yD,ϵ,ν,μ,β,ξ,width::Int,height::Int,pmig,pmat,preexisting,T,test_threshold)
    ...
    if preexisting == 1
        for t = 1:ceil(Int,5/σ)   # Longer burn-in time than initial program as we don't start from an estimate of the standing variation
            for i=1:width, j=1:height
                mated_females[i,j,:] = mating(females[i,j,:],males[i,j,:],pmat,mated_females[1:end-1]);
            end
            ### Throw away females which did not find a mate.
            ###mated_females = mated_females[1:end-1];

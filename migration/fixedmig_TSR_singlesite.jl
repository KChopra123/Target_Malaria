function fixedmig_TSR_singlesite(m,s,h,hN,σ,Rm,K,xD,yD,ϵ,ν,μ,β,ξ,width,height,pmig,preexisting,T,test_threshold,Gauss,terminate_resistance)
    ...
    # equivalent circular deme
        ...
        for i = trunc(Int,acrm/2)
            for j = trunc(Int,upn/2)
                x0[i,j,na] = xD;

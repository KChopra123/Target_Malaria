function sqmigration_TSR_mem(m,s,h,hN,σ,Rm,K,xD,yD,ϵ,ν,μ,β,ξ,width,height,D,pmig,area_approx,preexisting,T,test_threshold,Gauss,parallel,terminate_resistance)
...
# equivalent circular deme
    ...
    if preexisting == 1
        ...
        for tt = 1:Tburn
            println(tt)
            (x0,y0,Nf,Nm,N) = sqmignewfrequency(Rm,α,x0,y0,μ,Wf,Wm,na,ψ,mutmatrix,κ,Nf,Nm,Gauss,parallel,pmig);
        end
    ...
    for t = 1:T-1
        println(t)
        time[t+1] = t;
        #(xcurrent,ycurrent,Nf,Nm,N) = sqmignewfrequency(Rm,α,xcurrent,ycurrent,μ,Wf,Wm,na,ψ,mutmatrix,κ,Nf,Nm,Gauss,parallel,pmig);
        newfrequencies = sqmignewfrequency(Rm,α,xcurrent,ycurrent,μ,Wf,Wm,na,ψ,mutmatrix,κ,Nf,Nm,Gauss,parallel,pmig);
        ...
        #scalarresistf = scalarresistf/sum(Nf);
        #scalarresistm = scalarresistm/sum(Nm);
    ...
    #ynum = zeros(size(y));
    #for i = 1:na
    #    xnum[:,:,i,:] = x[:,:,i,:].*popsize[1,:,:,:];
    #    ynum[:,:,i,:] = y[:,:,i,:].*popsize[2,:,:,:];
    #end
    return (resistance,τ)

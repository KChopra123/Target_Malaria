function fixedmig_TSR_allee(m,s,h,hN,σ,Rm,K,xD,yD,ϵ,ν,μ,β,ξ,width,height,pmig,pmat,preexisting,T,test_threshold,Gauss,terminate_resistance)
...
# equivalent circular deme
    ...
    if preexisting == 1
        ...
        for tt = 1:Tburn
            println(tt)
            (x0,y0,Nf,Nm,N,~,~) = sqmignewfrequency_allee(Rm,α,x0,y0,μ,Wf,Wm,na,ψ,mutmatrix,κ,Nf,Nm,Gauss,pmig,pmat);
        end
    ...
    for t = 1:T-1
        ...
        newfrequencies = sqmignewfrequency_allee(Rm,α,xcurrent,ycurrent,μ,Wf,Wm,na,ψ,mutmatrix,κ,Nf,Nm,Gauss,pmig,pmat);

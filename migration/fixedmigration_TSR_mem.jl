function fixedmigration_TSR_mem(m,s,h,hN,σ,Rm,K,xD,yD,ϵ,ν,μ,β,ξ,width,height,pmig,preexisting,T,test_threshold,Gauss,terminate_resistance)
...
# equivalent circular deme
    ...
    if length(σ) == 1
        σ = σ*fill(1.0,(m,1));
    elseif length(σ) != m
        print("Error - σ should be a scalar or vector of length m")
        return (-3,-3)
    end
    ...
    for i = 1:acrm
        for j = 1:upn
            ...
            if x0[i,j,1] < 0 || y0[i,j,1] < 0
                ...
                println([i,j])
                x0[i,j,na] = x0[i,j,na] + x0[i,j,1];
                y0[i,j,na] = y0[i,j,na] + y0[i,j,1];
                x0[i,j,1] = 0;
                y0[i,j,1] = 0;
            end
        end
